...
def _make_temp_data(tmp_path):
    ...
    # Step 3: Xay dung DataFrame
    df = pd.DataFrame(X, columns=FEATURE_NAMES)
    df["target"] = y

    # Step 4: Luu 160 dong dau lam tap huan luyen, 40 dong cuoi lam tap danh gia
    train_path = str(tmp_path / "train.csv")
    eval_path  = str(tmp_path / "eval.csv")
    df.iloc[:160].to_csv(train_path, index=False)
    df.iloc[160:].to_csv(eval_path,  index=False)

    # Step 5: Tra ve (train_path, eval_path)
...
def test_metrics_file_created(tmp_path):
    ...
    # Step 8: Kiem tra file ton tai va noi dung
...
def test_model_file_created(tmp_path):
    ...
    # Step 9: Kiem tra file model ton tai
